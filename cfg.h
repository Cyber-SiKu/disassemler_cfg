...
// change to inst_t which support x86_64
// Need to include libdis.h for the insn_t use below. People will need to includes this anyway.
#include <xdisasm.h>
...
struct cfg_node_list {
    struct cfg_node_list *next;     // At beginning for list_append()
    struct cfg_node *node;
};

// Make a basic control flow graph for the inputted text
// This is a simple linear sweep.
void cfg_make(insn_t **insts, int insts_len, struct cfg_node_list **nodelist_ret, struct cfg_node_list **top_nodes_ret);
...
void cfg_fprint_graphviz_insts(FILE *outfile, struct cfg_node_list *node_list, insn_t **insts, int insts_len);
